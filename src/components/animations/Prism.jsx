...
const Prism = ({
  ...
  useEffect(() => {
    ...
    const SAT = transparent ? 1.5 : 1;
    // Lighter on mobile
    const SCALE = Math.max(0.001, isMobile ? Math.min(scale, 2.0) : scale);
    const HUE = hueShift || 0;
    const CFREQ = Math.max(0.0, colorFrequency || 1);
    const BLOOM = Math.max(0.0, bloom || 1);
    const RSX = 1;
    ...
    const renderer = new Renderer({
      ...
      alpha: transparent,
      antialias: false,
      // OGL passes through to WebGL context creation
    ...
    const fragment = /* glsl */ `
      ...
      void main(){
        ...
        const int STEPS = 80; // was 100 — small perf win
        for (int i = 0; i < STEPS; i++) {
          p = vec3(f, z);
          p.xz = p.xz * wob;
          p = uRot * p;
    ...
    // Page visibility pause (prevents hidden-tab burn)
    const onVisibility = () => {
    ...
    const render = (t) => {
      ...
      if (animationType === 'hover') {
        ...
        yaw = lerp(yaw, TY, inertia);
        pitch = lerp(pitch, TP, inertia);
        ...
        // rotate (default) — wobble done in shader
      }
    ...
    // Start loop; optionally suspend when offscreen
    if (suspendWhenOffscreen) {
      const io = new IntersectionObserver((entries) => {
        const vis = entries.some((e) => e.isIntersecting);
        if (vis) startRAF();
    ...
    return () => {
      ...
      // Force GL context loss to free GPU memory immediately
      try {
